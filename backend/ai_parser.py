...
SYSTEM_PROMPT = (
    f"현재 시스템의 기준 날짜는 {current_datetime}입니다. 사용자가 '오늘', '어제', '내일', '지난주' 등의 "
    f"상대적인 시간 표현을 사용하면 반드시 이 기준 날짜를 바탕으로 정확한 연/월/일로 계산하여 파싱하세요.\n\n"
) + """
...
## ★★★ 회의/행사 내용 5대항목 분류 규칙 ★★★
회의 및 행사 내용을 다음 5가지 대항목으로 분류하여 추출하세요. 해당 없는 항목은 빈 배열을 반환합니다.
1. background: 추진 배경 및 목적
2. agenda: 주요 안건 및 행사 내용
3. discussion: 세부 논의 및 운영 사항
4. conclusion: 주요 결과 및 성과
5. future_plan: 향후 추진 계획
모든 내용은 명사형으로 종결하는 개조식 문장이어야 합니다.
(background와 future_plan은 각각 위 JSON 스키마의 배열 필드에 작성하고, agenda·discussion·conclusion은 회의내용 필드에 통합하여 작성하세요.)
...
## [결측치 및 누락 정보 처리]
- 중요 정보 누락 시 `missing_warnings`에 메시지를 작성하되, '추진본부', '실행부서', '추진과제', '세부과제', '과업', '세부과업'이 누락된 경우는 절대 경고를 띄우지 말고 무시하세요.
- 경고 메시지 작성 시 예시(ex) 부분은 반드시 HTML <b> 태그를 써서 볼드 처리하세요.
  * 참석자 누락 시: "참석자 정보를 제공해주세요<br/><b>ex) 홍길동 직원 글로벌추진팀</b><br/><b>ex) 김갑수 학생 AI빅데이터학과 260545</b>"
  * 회의 일시 누락 시: "회의 일시를 입력해주세요 <b>ex) 2026.05.08. 11:00~15:00</b>"
  * 결제 일시 누락 시: "결제일시를 입력해주세요 <b>ex) 2026.05.05. 16:32</b>"
